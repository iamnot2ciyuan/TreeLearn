...
def run_treelearn_pipeline(config, config_path=None):
    ...
    pointwise_results = get_pointwise_preds(model, dataloader, config, logger)
    ...
    # ================= [核心修改：斩断内存黑洞，释放高维特征] =================
    del backbone_feats
    gc.collect()  # 强行通知操作系统回收这几个 GB 的物理内存
    # 构造轻量级假数组占位。由于 ensemble 只需要合并特征，N×1 的 float16 数组极大降低了合并的内存开销与时间
    fake_backbone_feats = np.zeros((len(coords), 1), dtype=np.float16)
    # =========================================================================
    ...
    data = ensemble(coords, semantic_prediction_logits, semantic_labels, offset_predictions, 
                    offset_labels, instance_labels, fake_backbone_feats, input_feats)  # 传入 fake_backbone_feats
    ...
    instance_preds = get_instances(
        ...
        config.grouping,
        input_feats[:, -1],
        tree_class_in_dataset,
        NON_TREES_LABEL_IN_GROUPING,
    ...
    if config.save_cfg.save_pointwise:
        ...
        pointwise_results = {
            ...
            'backbone_feats': backbone_feats, # 这里保存的是极小体积的占位符，节省了大量硬盘空间
        ...
        # Offset-shifted coordinates for initial clustering results before remaining-point assignment.
        # Only keep actual clustered trees here; non-tree (0) and not-assigned (-1) points are not instances yet.
        sem_mask = instance_preds_after_initial_clustering > NON_TREES_LABEL_IN_GROUPING
        cluster_coords = coords[sem_mask] + offset_predictions[sem_mask]
        cluster_coords = np.hstack([cluster_coords, instance_preds_after_initial_clustering[sem_mask].reshape(-1, 1)])
