...
def _resolve_inference_config(config):
    """Support both legacy model cfg and full pipeline cfg."""
    if hasattr(config, 'model'):
        return config.model, bool(getattr(config, 'fp16', False))
    return config, False
...
def get_pointwise_preds(model, dataloader, config, logger=None):
    model_cfg, use_fp16 = _resolve_inference_config(config)
    model.eval()
    ...
    with torch.inference_mode():
        for batch_idx, batch in enumerate(tqdm.tqdm(dataloader), start=1):
            # get voxel_sizes to use in forward
            batch['voxel_size'] = model_cfg.voxel_size
            # forward
            try:
                with torch.cuda.amp.autocast(enabled=use_fp16):
                    output = model(batch, return_loss=False)
                offset_prediction = output['offset_predictions'].cpu()
                semantic_prediction_logit = output['semantic_prediction_logits'].cpu()
                backbone_feat = output['backbone_feats'].cpu()
            ...
            del output, offset_prediction, semantic_prediction_logit, backbone_feat
            # Reduce allocator fragmentation during long-running inference loops.
            if batch_idx % 5 == 0:
